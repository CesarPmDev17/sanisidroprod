...
import 'package:app_san_isidro/modules/salud/doctor/salud_doctor_controller.dart';
...
class SaludDoctorPage extends StatelessWidget {
  final _conX = Get.put(SaludDoctorController());
            ...
            child: SafeArea(
              child: Column(
                ...
                children: [
                  ...
                  SaludTitleScaffold(
                    title: 'Ahora,',
                    subTitle: 'selecciona un(a) doctor(a)',
                  ),
                  Obx(
                    () => IgnorePointer(
...
class _EntityList extends StatelessWidget {
  final SaludDoctorController _conX;
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<SaludDoctorController>(
      id: _conX.gbList,
      builder: (_) {
        List<Doctor> list = [];
        if (_conX.searchText.value.isNotEmpty) {
          list = _conX.list
              .where((item) => (item.txtpersonasalud.toLowerCase())
